...
// title, description, status, owner, member, label, manHour, dueAt, startAt, endAt, createdAt, updatedAt
...
class CsvDataImporter {
  ...
  insertRowAsCard(items, sort) {
    ...
    for (let i=0; i<items.length; i++) {
      ...
      } else if (i === index.dueAt) {
        const d = new Date(`${item} 12:00:00`);
        if(!isNaN(d.getTime())) card.dueAt = d;
      } else if (i === index.startAt) {
        const d = new Date(`${item} 12:00:00`);
        if(!isNaN(d.getTime())) card.startAt = d;
      } else if (i === index.endAt) {
        const d = new Date(`${item} 12:00:00`);
        if(!isNaN(d.getTime())) card.endAt = d;
      }
    }
  ...
  exportCards(filter, boardId, tsv) {
    ...
    const headers = ['title', 'description', 'status', 'owner', ...hMembers, ...hLabels,
                     'manHour', 'dueAt', 'startAt', 'endAt', 'createdAt', 'updatedAt'];
    ...
    cards.forEach((card) => {
      ...
      const fields = [`"${card.title}"`,
                      ...
                      ...labels,
                      card.manHour || '',
                      card.dueAt ? moment(card.dueAt).format('YYYY-MM-DD') : '',
                      card.startAt ? moment(card.startAt).format('YYYY-MM-DD') : '',
                      card.endAt ? moment(card.endAt).format('YYYY-MM-DD') : '',
